...
class MyWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.leftGraphWidget = CustomWidgets.plot.myImageView(
            topLeft, view=pg.PlotItem()
        )
        self.leftGraphWidget.show()
        self.rightGraphWidget = CustomWidgets.plot.myImageView(
    ...
    def startRemoveMesh(self):
        if self.leftConfCheckBox.isChecked():
            ...
            for image_index in range(len(self.rawData)):
                newImage = CompFunc.fft.autoRemoveFourierPeaks(
                    self.rawData[image_index]
                )
                # newImage=CompFunc.fft.autoRemoveFourierPeaks(newImage)
                self.meshPattern = CompFunc.fft.getRatio(
                ...
                self.smallImageArray = CompFunc.fft.splitLargeImage(
                    self.meshPattern[0], M, N, blockSize
                )
                self.avgScanImageArray = CompFunc.fft.splitLargeImage(
                ...
                if __name__ == "__main__":
                    with Pool(processes=CPU_NUM) as pool:
                        ...
                        while (index + CPU_NUM) < M * N:
                            m_Results = [
                                pool.apply_async(
                                    CompFunc.fft.optimizeMeshFromData,
                                    args=(
                ...
                self.meshPattern[0] = CompFunc.fft.constructLargeImage(
    ...
    def rightFinish(self):
        ...
        for i in np.arange(0, np.size(self.rawData, 0), 1):
            self.fixedData[i] = CompFunc.fft.fixCurvature2D(
