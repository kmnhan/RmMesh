...
def dataToFft(inputdata):
    fftData = np.zeros(np.shape(inputdata), dtype=np.complex128)
    M = len(inputdata[0]) // 2
...
def fftToData(fftData, rawIntensity):
    reconstructData = np.zeros(np.shape(fftData), dtype=np.complex128)
...
def genHigherPeaks(firstOrder):
    peaks = np.zeros((25, 2), dtype=np.int64)
...
def recreateMeshFft(fftData, meshPattern, paraList):
    ...
    sum_expanded = np.zeros(len(paraList), dtype=np.complex128)
...
def autoRemoveFourierPeaks(image):
    ...
    M = len(image) // 2
    N = len(image[0]) // 2
    peaks = np.zeros((3, 2), dtype=np.int64)
    peaks[0, 0] = M
    peaks[0, 1] = N
...
def optimizeMeshFromData(meshData, rawImage):
    ...
    if sum > 0.0:
        ...
        recreatedMesh = np.zeros(meshData.shape, dtype=np.complex128)
